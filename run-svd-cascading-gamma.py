...
if i_task == 1:
    ...
    pickle_key =   ('d3', 'svals__individual_layer__gammas80')
    pickle_key =   ('temp', 'svals__l0__16')
...
# if i_task == 1:
#     model=model_dict[d3_tag]
#     pickle_key =   ('d3', 'svals__m1_to_1__cascading_gamma__gammas80')
    
#     if LRP:
#         mat_funcs = funcs_cascading__d3__m1_to_1(model)
#         mask = np.any(model.forward(data).detach().numpy(), axis=1)
#         d = data[mask][:20]
...
if LRP:
    ...
    calc_mats_batch_functional(mat_funcs, gammas80, d, pickle_key=pickle_key, overwrite=True)
