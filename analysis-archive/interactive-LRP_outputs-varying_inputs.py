...
def update(_):
    ...
    all_X, W, all_h, all_R1, all_C, all_R0 = \
        h0_W_h1_R1_C_R0(w11=w11, w12=w12, A1=np.array([in1]))
    
    print(W)
    print(all_C[0].T)
    print()
    
    all_X = np.array([A1, 1-A1])
    all_h = W @ all_X
    all_R0 = all_C[0].T @ all_h
...
# Control in1 for which the LRP coefficients get calculated:
ax_h2 = plt.axes([0.25, 0.1, 0.65, 0.03], facecolor=axcolor)
in1_slider = Slider(
    ax=ax_h2,
    label='in_1',
    valmin=valmin,
